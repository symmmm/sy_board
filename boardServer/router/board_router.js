...
module.exports = (secret) => {
        ...
        if (token_check) {
            ...
            await query.count((err, count) => {
                if (err) {
                    ...
                    new_Board.post_date = moment().utcOffset('+0900').format('YYYY-MM-DD HH:mm:ss');
    ...
    router.get('/search/author/:post_author', async (req, res) => {
        ...
        await Board.find({ post_author: author, post_yn: 'y' },
            (err, boards) => {
                if (err) {
                    console.log(err);
    ...
    router.post('/countBoard', async (req, res) => {
        ...
        await Board.countDocuments({ post_date: { $gte: start_date, $lt: end_date }, post_yn: 'y' }, (err, count) => {
            if (err) {
        ...
        for (let i = 0; i < fin_code.length; i++) {
            await Board.countDocuments({ 'post_fin_list.code': fin_code[i], post_date: { $gte: start_date, $lt: end_date }, post_yn: 'y' }, (err, count) => {
                if (err) {
                    console.log("CountBoard_ERROR");
                    res.json({ "countBoard": -1 });
                }
                    ...
                    fin_count_array.push({ fin_code: fin_code[i], fin_count: count, total_count: totalCount });
                }
            })
        }
    ...
    router.post('/chart', async (req, res) => {
        ...
        for (let i = 4; i >= 0; i--) {
            ...
            await Board.countDocuments({ post_date: { $gte: temp_start_date, $lt: temp_end_date }, post_yn: 'y' }, (err, count) => {
                if (err) {
                    console.log("CountBoard_ERROR");
                    res.json({ "countBoard": -1 });
                }
            ...
            await Board.countDocuments({ 'post_fin_list.name': fin_name, post_date: { $gte: temp_start_date, $lt: temp_end_date }, post_yn: 'y' }, (err, count) => {
                if (err) {
                    console.log("CountBoard_ERROR");
                    res.json({ "countBoard": -1 });
                }
                    ...
                    chart_data.push({ name: moment(temp_start_date).format('YYYY-MM-DD'), data: data });
                }
            })
        }
    ...
    router.get('/today', (req, res) => {
        ...
        Board.aggregate([
            {
                $match: { post_date: { $gte: start_date, $lt: end_date }, post_yn: 'y' }
            },
            {
